...
MapServerRenderer::MapServerRenderer(const QString &mapPath, QObject *parent)
    : TileRenderer(parent)
{
    QFile pathFile(QDir(CUTEMAP_DIR).filePath("data/mapserver-path.map"));
...
{
    QString mapFile = QDir(CUTEMAP_DIR).filePath("data/mapserver.map");
    QString query = QString("map=%1&mode=tile&tile=%2+%3+%4").arg(mapFile).arg(x).arg(y).arg(z);

    QProcess server;
    server.start(QDir(MAPSERVER_DIR).filePath("build/mapserv"),
